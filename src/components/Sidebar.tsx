  ...
  return (
    <aside
      className={`bg-gradient-to-b from-[#0B0080] via-[#0D009C] to-[#0051BA] flex flex-col justify-between shadow-[8px_8px_24px_rgba(0,29,66,0.15)] border border-primary/20 overflow-hidden shrink-0 transition-all duration-300 ease-in-out z-50
        fixed md:relative inset-y-0 left-0 md:translate-x-0 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
        } ${isCollapsed ? 'w-64 md:w-20' : 'w-64'} ${isMobileOpen ? 'rounded-none shadow-2xl h-full' : 'rounded-[24px]'
        }`}
    >
      ...
      <div className="relative z-10">
        ...
        <div
          className={`border-b border-white/10 flex transition-all duration-300 ${isCollapsed
        ...
        >
          <div className="flex items-center gap-3">
            ...
            />
            <div
              className={`flex flex-col transition-all duration-300 ease-in-out whitespace-nowrap overflow-hidden ${isCollapsed ? 'md:max-w-0 md:opacity-0 md:pointer-events-none' : 'max-w-[150px] opacity-100'
                }`}
            >
              <span className="font-display font-bold text-white tracking-wide text-md">
                <span style={{ color: '#FFDA1A' }}>U</span>friend CX
              </span>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <svg
                className={`w-3.5 h-3.5 transform transition-transform duration-300 ease-out ${isCollapsed ? 'rotate-180' : ''
                  }`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
        ...
        <nav className={`p-4 space-y-1.5 ${isCollapsed ? 'flex flex-col items-center' : ''}`}>
          ...
          <button
            ...
            title="แดชบอร์ด"
            className={`flex items-center rounded-xl text-xs font-semibold tracking-wide transition-all duration-300 border-l-4 ${isCollapsed ? 'w-10 h-10 justify-center p-0 border-l-0' : 'w-full gap-3 px-4 py-2.5'
          ...
          >
            ...
            <span
              className={`transition-all duration-300 ease-in-out whitespace-nowrap overflow-hidden ${isCollapsed ? 'max-w-0 opacity-0 pointer-events-none' : 'max-w-[150px] opacity-100'
                }`}
            >
              แดชบอร์ด
            </span>
      ...
      <div
        className={`p-4 border-t border-blue-900/30 relative z-10 transition-all duration-300 ${isCollapsed ? 'flex justify-center' : ''
          }`}
      >
        <div className="flex items-center gap-3">
          ...
          <div
            className={`transition-all duration-300 ease-in-out whitespace-nowrap overflow-hidden ${isCollapsed ? 'max-w-0 opacity-0 pointer-events-none' : 'max-w-[180px] opacity-100'
              }`}
          >
